...
- criada IndexedDB separada `conta-de-casa-market-visual-catalog`, com stores `products` e `meta`;
- cada produto é identificado e deduplicado por `marketId|pid`;
- o índice guarda nome, embalagem, categorias, página oficial e timestamps, mas **não guarda preço**;
- adicionadas 12 categorias: Bebidas, Lacticínios e ovos, Frutas e legumes, Carne e peixe, Padaria e pastelaria, Mercearia/Despensa, Congelados, Snacks e doces, Higiene pessoal, Limpeza, Bebé e Animais;
- descoberta limitada às fontes já suportadas de Continente e Pingo Doce via `cesta.pt`;
- só são aceites resultados com PID e URL oficial coerentes;
- botão **Ver preço atual** reutiliza `#marketCatalogSearch` e a pesquisa viva existente;
- adicionados filtros Todos / Continente / Pingo Doce e estatísticas de produtos/imagens;
- fallback local mantém o cartão estável enquanto não existe imagem oficial validada;
- grelha usa 3 colunas em desktop, 2 até 680 px e 1/composição lateral até 350 px.

### Acumulação controlada

- máximo de 18 pesquisas por sessão e 48 por dia;
...
- resolvedor direto de imagens com concorrência máxima 2;
- enriquecimento suspenso quando offline, página oculta ou `Save-Data` ativo;
- cursor persistente permite continuar a descoberta entre sessões.

A arquitetura permite crescimento gradual para centenas/milhares de SKUs ao longo do uso, sem alegar cobertura integral instantânea dos catálogos dinâmicos.

### Fotografias

- o resolvedor direto valida a URL exata da página com `CDCOfficialMarketImages.safeProductUrl`;
- candidatos de fotografia continuam a ser validados por `safeOfficialImageUrl` e PID exato;
- imagens válidas são persistidas pela `75-image-library1`, com TTL de 45 dias;
- se a resolução direta falhar, o resolvedor oficial anterior permanece como fallback;
- nenhum binário de imagem é copiado para GitHub.

### Segurança e integridade

- novos módulos não referenciam `appState`, `saveState()`, `commit()`, `estimatedCents`, `actualCents` ou `amountCents`;
- `core.js`, `finance.js`, `STATE_VERSION = 5`, pagamentos, faturas, PIN, PBKDF2-SHA-256, AES-GCM, QR e sincronização cifrada permanecem inalterados;
- o catálogo visual não persiste preços e a fotografia continua independente do preço/transação.

### QA e distribuição

- criado `tests/market-visual-catalog.test.cjs` e integrado em CI/Pages;
- o primeiro CI revelou apenas uma regex incorreta no próprio teste; a asserção foi corrigida;
- CI técnico verde no SHA `8e5d61c3c68771cd3e1cd5990cbe201e30fb7baa`;
- CI após documentação e release manifest verde no SHA `c45b7be38748c22f21c8168fd0edced8c8cc0987`;
...
- publicação em `main` permanece pendente até integração + CI + Pages do SHA final.
