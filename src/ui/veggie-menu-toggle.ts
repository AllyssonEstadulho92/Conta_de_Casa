/* Conta de Casa v76 — Veggie Burger em TypeScript.
   Camada visual incremental: duas linhas -> X com animação determinística,
   preservando o controlador funcional v73 do drawer. */
...
type MenuVisualState = 'open' | 'closed';
...
(function installVeggieMenuToggle(root: Window) {
  ...
  function install(): void {
    ...
    if (!sentinel) sentinel = button.querySelector<SVGSVGElement>(':scope > svg.ui-icon-svg');
    if (!sentinel) {
    ...
    const moveToDrawerOverlay = (): void => {
      ...
      if (button.parentElement !== drawer) drawer.insertBefore(button, drawerShell);
      button.classList.add('drawer-menu-control', 'drawer-menu-overlay-control');
    };

    const animateGlyph = (state: MenuVisualState): void => {
      if (prefersReducedMotion() || typeof upperLine.animate !== 'function' || typeof lowerLine.animate !== 'function') return;
      ...
      glyph.animate(
        opening
          ...
          : [{ transform: 'scale(1.02)' }, { transform: 'scale(.96)', offset: .55 }, { transform: 'scale(1)' }],
        timing
      );
    };

    let previousState: MenuVisualState = button.getAttribute('aria-expanded') === 'true' ? 'open' : 'closed';

    const syncVisualState = (withAnimation = true): void => {
      const state: MenuVisualState = button.getAttribute('aria-expanded') === 'true' ? 'open' : 'closed';
      ...
      if (state !== previousState) {
        if (withAnimation) animateGlyph(state);
    ...
    const syncSoon = (): void => schedule(() => syncVisualState(true));
