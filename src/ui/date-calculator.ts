...
type DateCalculatorApi = Readonly<{
  revision: string;
...
type DateCalculatorRoot = typeof globalThis & {
  CDCIcons?: { markup?: (name: string, size?: number) => string };
  CDCDateCalculator?: DateCalculatorApi;
};

(function installDateCalculator(root: DateCalculatorRoot) {
  ...
  function supportedDateKey(value: string): string {
    ...
    return parts && parts.year >= MIN_YEAR && parts.year <= MAX_YEAR ? key : '';
  }
  ...
  function dayNumber(dateKey: string): number {
    ...
    return key ? civilDayNumber(key) : Number.NaN;
  }
  ...
  function calendarSpan(startInput: string, endInput: string): CalendarSpan | null {
    ...
    const afterYearParts = parseCivilDateKey(afterYears);
    if (!afterYearParts) return null;
    let months = (toParts.year - afterYearParts.year) * 12 + (toParts.month - afterYearParts.month);
    ...
    return Number.isInteger(days) && days >= 0 ? {years,months,days} : null;
  }
  ...
  function difference(
    ...
    if (!Number.isInteger(signed) || !range || !span) return null;
  ...
  function businessDays(
    ...
    return {start,end,selectedDays:range.count,businessDays:business,weekendDays:range.count-business};
  }
  ...
  function addDays(baseInput: string,amount: number,mode: 'calendar'|'business'='calendar'): string {
    ...
    return supportedDateKey(addCivilDays(base,amount));
  }

  const API: DateCalculatorApi = Object.freeze({
    revision:REVISION,difference,businessDays,addDays,calendarSpan,dayOfYear,weekdayName
  });
  root.CDCDateCalculator=API;

  const maybeDocument = typeof document==='undefined' ? null : document;
  if (!maybeDocument) return;
  const doc: Document = maybeDocument;
  ...
  function iconMarkup(name:string,size=20):string {
    try { return root.CDCIcons?.markup?.(name,size) || icon(name,size); }
    catch (_error) { return ''; }
  }
  function formatDateLong(dateKey:string):string {
    ...
    return new Intl.DateTimeFormat('pt-PT',{day:'2-digit',month:'long',year:'numeric',timeZone:'UTC'}).format(date);
  }
  ...
  function todayKey():string {
    return supportedDateKey(currentLocalDateKey());
  }
  ...
  function setMode(mode:CalculatorMode,focusTab=false):void {
    ...
    queryAll<HTMLElement>('[data-datecalc-panel]',dialog).forEach(panel=>{panel.hidden=panel.dataset.datecalcPanel!==mode;});
    resetResult();
  }
  ...
  function calculateDifference():void {
    const result=difference(formValue('cdcDateDiffStart'),formValue('cdcDateDiffEnd'),checked('cdcDateDiffIncludeStart'),checked('cdcDateDiffIncludeEnd'));
    if(!result){publishError('Escolha duas datas válidas entre 01/01/1900 e 31/12/9999.');return;}
    ...
    const details:Array<[string,string]>=[
      ...
      ['Semanas completas',`${plural(result.weeks,'semana','semanas')} + ${plural(result.remainingDays,'dia','dias')}`],
    ...
    const note='“Dias decorridos” é a diferença matemática. A contagem selecionada respeita as opções de inclusão. O período civil decompõe anos completos, depois meses completos com ajuste ao último dia válido do mês.';
  ...
  function calculateAdd():void {
    ...
    const amount=Number(formValue('cdcDateAddAmount'));
    const direction=Number(formValue('cdcDateAddDirection'))===-1?-1:1;
    const mode=formValue('cdcDateAddMode')==='business'?'business':'calendar';
    if(!base||!Number.isInteger(amount)||amount<0||amount>MAX_ADD_DAYS){publishError(`Escolha uma data válida e uma quantidade inteira entre 0 e ${MAX_ADD_DAYS}.`);return;}
    const result=addDays(base,amount*direction,mode);
    if(!result){publishError('O resultado ultrapassa o intervalo de datas suportado.');return;}
    ...
    const details:Array<[string,string]>=[
      ...
      ['Operação',`${plural(amount,'dia','dias')} ${verb}`],
      ['Tipo',type],
      ['Data resultante',formatDateWithWeekday(result)],
      ['Dia do ano',String(dayOfYear(result))]
    ];
    const note=mode==='business'?'Sábados e domingos são ignorados. Feriados não são presumidos nem descontados.':'A soma usa dias civis inteiros; horas e fuso horário não participam.';
    publishResult(resultShell(formatDateLong(result),'Data exata',details,note),`${formatDateLong(base)} ${direction>0?'+':'−'} ${amount} ${type}: ${formatDateLong(result)} (${weekdayName(result)}).`);
  }
  function calculateBusiness():void {
    const result=businessDays(formValue('cdcDateBusinessStart'),formValue('cdcDateBusinessEnd'),checked('cdcDateBusinessIncludeStart'),checked('cdcDateBusinessIncludeEnd'));
    if(!result){publishError('Escolha duas datas válidas entre 01/01/1900 e 31/12/9999.');return;}
    const details:Array<[string,string]>=[
    ...
    publishResult(resultShell(plural(result.businessDays,'dia útil','dias úteis'),'Contagem exata segundo a regra definida',details,note),`Entre ${formatDateLong(result.start)} e ${formatDateLong(result.end)}: ${result.businessDays} dias úteis, considerando segunda a sexta-feira e as opções de inclusão selecionadas.`);
  }
  ...
  function swap(kind:'difference'|'business'):void {
    const start=byId<HTMLInputElement>(kind==='difference'?'cdcDateDiffStart':'cdcDateBusinessStart');
    const end=byId<HTMLInputElement>(kind==='difference'?'cdcDateDiffEnd':'cdcDateBusinessEnd');
    if(!start||!end)return;
    const value=start.value;start.value=end.value;end.value=value;
  }
  ...
  async function copyText(text:string):Promise<boolean> {
    ...
    try {await navigator.clipboard.writeText(text);return true;}
      ...
      doc.body.appendChild(textarea);textarea.select();
      let copied=false;try{copied=doc.execCommand('copy');}catch(_copyError){copied=false;}
      textarea.remove();return copied;
    }
  }
  async function copyResult():Promise<void> {
    const ok=await copyText(lastResultText);
    const label=query<HTMLElement>('[data-datecalc-copy] span');
    if(!label)return;
    label.textContent=ok?'Copiado':'Não foi possível copiar';
    setTimeout(()=>{label.textContent='Copiar resultado';},1600);
  }
  async function shareResult():Promise<void> {
    if(!lastResultText)return;
    if(typeof navigator.share==='function'){
      try{await navigator.share({title:'Calculadora de datas · Conta de Casa',text:lastResultText});return;}
  ...
  function printResult():void {
    ...
    root.print();setTimeout(cleanup,1200);
  }
  ...
  function installInteractions():void {
    doc.addEventListener('click',event=>{
      ...
      if(tab){event.preventDefault();const mode=tab.dataset.datecalcMode as CalculatorMode|undefined;if(mode)setMode(mode);return;}
      const todayButton=target.closest<HTMLButtonElement>('[data-datecalc-today]');
      if(todayButton){event.preventDefault();const input=byId<HTMLInputElement>(todayButton.dataset.datecalcToday??'');if(input){input.value=todayKey();input.focus({preventScroll:true});}return;}
      const swapButton=target.closest<HTMLButtonElement>('[data-datecalc-swap]');
      if(swapButton){event.preventDefault();const kind=swapButton.dataset.datecalcSwap;if(kind==='difference'||kind==='business')swap(kind);return;}
      const calculateButton=target.closest<HTMLButtonElement>('[data-datecalc-calculate]');
      if(calculateButton){event.preventDefault();const mode=calculateButton.dataset.datecalcCalculate as CalculatorMode|undefined;if(mode)calculate(mode);return;}
    ...
    const dialog=ensureDialog();
    dialog?.addEventListener('close',()=>{if(lastLauncher?.isConnected)lastLauncher.focus({preventScroll:true});lastLauncher=null;});
    dialog?.addEventListener('click',event=>{if(event.target===dialog)closeDialog();});
  }

  function observeArchitecture():void {
    const settingsPage=byId('page-settings');
    if(settingsPage){launcherObserver=new MutationObserver(ensureLauncher);launcherObserver.observe(settingsPage,{childList:true,subtree:true});}
    const app=byId('app');
    if(app){appHiddenObserver=new MutationObserver(()=>{if(app.hidden)closeDialog();});appHiddenObserver.observe(app,{attributes:true,attributeFilter:['hidden']});}
  }
  function start():void {
    doc.documentElement.dataset.dateCalculator=REVISION;
    ensureDialog();ensureLauncher();installInteractions();observeArchitecture();
    requestAnimationFrame(ensureLauncher);setTimeout(ensureLauncher,120);
  }
  if(doc.readyState==='loading')doc.addEventListener('DOMContentLoaded',start,{once:true});else start();
})(globalThis as DateCalculatorRoot);
