...
assert.match(brandCss,/\.market-product-photo[\s\S]*display:grid!important/,'v74 presentation must expose verified product photos');
...
assert.doesNotMatch(sw,/\.\/ui-consistency\.css/,'obsolete visual layer must not ship in v74');
...
console.log('Market real/official images remain isolated and visible under the v74 bundle: OK');
