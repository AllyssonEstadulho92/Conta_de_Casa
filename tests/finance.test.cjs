...
const monthKeysBeforeHistory=JSON.parse(JSON.stringify(vm.runInContext("Object.keys(appState.months).sort()",context)));
const history=JSON.parse(JSON.stringify(vm.runInContext("monthlySpendHistory('2026-09',2)",context)));
const monthKeysAfterHistory=JSON.parse(JSON.stringify(vm.runInContext("Object.keys(appState.months).sort()",context)));
assert.deepEqual(monthKeysAfterHistory,monthKeysBeforeHistory,'reading spend history must not create empty month profiles');
...
// Budget allocation follows the bill month, while cash-flow reporting follows paidAt.
vm.runInContext(`
...
assert.equal(sepAfterEarlyOctoberPayment.paymentTotal,17800,'cash flow must keep the early payment in September');
assert.equal(sepAfterEarlyOctoberPayment.budgetPaymentTotal,5800,'September budget must not absorb an October bill');
...
assert.equal(octAfterEarlyPayment.paymentTotal,0,'October cash flow must not invent a payment date');
assert.equal(octAfterEarlyPayment.budgetPaymentTotal,12000,'October budget must reflect its paid bill even when paid early');
