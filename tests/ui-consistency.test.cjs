...
assert.match(menuCss,/Conta de Casa v71/,'v72 preserves the validated v71 menu visual layer');
...
assert.match(menuJs,/drawer\.close=animatedDrawerClose/,'controller must preserve the dialog during the off-canvas exit transition');
assert.match(menuJs,/const source=shell\.contains\(event\.target\)\?'drawer':'page'/,'v72 must accept page/backdrop swipe as a closing gesture source');
assert.match(menuJs,/prefersReducedMotion/,'menu JS motion must respect reduced-motion preference');
...
try{
  ...
  assert.ok(index.indexOf('market-shopping-focus.css')<index.indexOf('mobile-menu-toggle.css'),'v72 global menu layer must remain the final mobile-menu layer');
...
console.log('Authoritative typography, Lucide, navigation, v66 shell, v65 market focus and v72 mobile menu consistency tests: OK');
