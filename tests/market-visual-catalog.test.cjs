...
assert.match(catalog,/function dayKey\(now=new Date\(\)\)/,'daily catalog budget must use an explicit civil-date helper');
assert.match(catalog,/date\.getFullYear\(\)[\s\S]*date\.getMonth\(\)\+1[\s\S]*date\.getDate\(\)/,'daily catalog budget must follow the device local calendar');
assert.doesNotMatch(catalog,/toISOString\(\)\.slice\(0,10\)/,'daily catalog budget must not roll over on UTC midnight');
