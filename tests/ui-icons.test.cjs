...
assert.match(pages,/design-system\.css/,'Pages distribution must include the consolidated v74 visual system');
assert.match(pages,/v74-experience\.css/,'Pages distribution must include the prototype composition layer');
assert.doesNotMatch(pages,/['"]ui-consistency\.css['"]/,'obsolete visual normalization layer must not ship');
assert.doesNotMatch(pages,/['"]v64-runtime\.css['"]/,'obsolete v64 shell CSS must not ship');
