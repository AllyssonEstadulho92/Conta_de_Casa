...
assert.match(js, /clamp\(width\+Math\.min\(0,dx\),0,width\)/);
assert.match(js, /clamp\(Math\.max\(0,dx\),0,width\)/);
...
assert.match(css, /transform \.3s cubic-bezier\(\.32,\.72,0,1\)/);
assert.match(css, /var\(--drawer-drag-x,100%\)/);
assert.match(css, /border-left:1px solid/);
assert.match(css, /box-shadow:-18px 0 40px/);
assert.match(css, /\.drawer-nav \.nav-btn::before\{[\s\S]*inset:10px 0 10px auto/);
...
/* v74 changes the release and visual identity, not the validated v73 menu controller. */
...
console.log('v73 right-side navigation remains protected inside the v74 release.');
