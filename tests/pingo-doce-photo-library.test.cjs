...
assert.match(source,/function dayKey\(now=new Date\(\)\)/,'Pingo Doce daily budget must use an explicit civil-date helper');
assert.match(source,/date\.getFullYear\(\)[\s\S]*date\.getMonth\(\)\+1[\s\S]*date\.getDate\(\)/,'Pingo Doce daily budget must follow the device local calendar');
assert.doesNotMatch(source,/toISOString\(\)\.slice\(0,10\)/,'Pingo Doce daily budget must not roll over on UTC midnight');
