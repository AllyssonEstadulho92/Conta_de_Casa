...
assert.match(html,/Prioridade de pagamentos/,'Dashboard must expose a payment-priority section');
assert.match(html,/Atrasadas primeiro, depois por vencimento\./,'priority header must explain that overdue bills come first');
...
assert.match(render,/function dashboardPriorityConfig\(bill,index,now=new Date\(\)\)/,'Dashboard priority labels must depend on the real bill status and due date');
...
assert.match(render,/priorityBills\.map\(\(bill,index\)=>dashboardUpcomingBillHtml\(bill,index,priorityBills\.length\)\)/,'priority bills must render their rank after canonical due-date sorting');
assert.match(render,/priorityBills = n\.bills\.filter\(b=>\{const days=billDaysUntil\(b\);return remainingForBill\(b\)>0&&Number\.isFinite\(days\);\}\)\.sort\(compareBillsByDue\)/,'overdue and upcoming unpaid bills must share one due-date queue');
assert.doesNotMatch(render,/priorityBills[^\n]*days>=0/,'overdue bills must not be excluded from the payment-priority queue');
assert.match(render,/status==='overdue'[\s\S]*tone:'pay',label:'Pagar agora'/);
assert.match(render,/days===0[\s\S]*tone:'pay',label:'Vence hoje'/);
assert.match(render,/days<=3[\s\S]*tone:'next',label:'Prioridade'/);
assert.match(render,/days<=7[\s\S]*tone:'soon',label:'A seguir'/);
