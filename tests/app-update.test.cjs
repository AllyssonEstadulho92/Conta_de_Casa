...
assert.doesNotMatch(sw,/applyRequested=true/,'service worker must not own reload state');
assert.doesNotMatch(sw,/client\.navigate\(client\.url\)/,'service worker must never navigate open clients during activation');
assert.match(events,/reloadForNewBuildWhenSafe/,'the page runtime must remain the single authority for safe update reloads');
...
assert.match(sw,/no-client-navigate1/,'PWA cache must identify the single-authority reload fix');
...
for(const asset of ['app-update.css','v76-version-about.css','app-update.js','design-system.css','v64-runtime.js','market-shopping-focus.css','market-shopping-focus.js','mobile-menu-toggle.css','mobile-menu-toggle.js','v75-architecture.css','v76-planning-more.css','v75-architecture.js','v75-stability.css','v75-stability.js','v75-layout-polish.css','v75-drawer-theme.css','invoice-extractor.js','invoice-ocr.js','invoice-extraction-rules.json','release-manifest.json','vendor/zxing-browser.min.js','vendor/ZXING_LICENSE.txt'])assert.ok(publicFilesBlock.includes(`'${asset}'`),`${asset} must be copied to dist`);
...
try {
  ...
  assert.match(index,/script-src 'self' 'wasm-unsafe-eval'; worker-src 'self';/,'published CSP must keep OCR WebAssembly and workers same-origin');
  assert.doesNotMatch(index,/unpkg\.com/);
  assert.match(index,/name="barcode-reader-src" content="\.\/vendor\/zxing-browser\.min\.js"/);
  assert.match(events, /\.\/sw\.js\?v=76-invoice-hierarchy-ocr7/);
  ...
  for(const asset of ['app-update.css','v76-version-about.css','app-update.js','design-system.css','v64-runtime.js','market-shopping-focus.css','market-shopping-focus.js','mobile-menu-toggle.css','mobile-menu-toggle.js','v75-architecture.css','v76-planning-more.css','v75-architecture.js','v75-stability.css','v75-stability.js','v75-layout-polish.css','v75-drawer-theme.css','invoice-extractor.js','invoice-ocr.js','invoice-extraction-rules.json','release-manifest.json','vendor/zxing-browser.min.js','vendor/ZXING_LICENSE.txt','vendor/ocr/tesseract.min.js','vendor/ocr/worker.min.js','vendor/ocr/lang/por.traineddata.gz'])assert.ok(fs.existsSync(path.join(dist,asset)),`${asset} must exist in dist`);
