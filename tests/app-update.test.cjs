...
assert.equal(releaseManifest.latestVersion,'v66');
assert.ok(Array.isArray(releaseManifest.releases));
assert.equal(releaseManifest.releases[0].version,'v66');
...
assert.ok(releaseManifest.releases[0].items.some(item=>/fundo|cor/i.test(item)),'v66 notes must expose the unified mobile shell colour');
assert.ok(releaseManifest.releases[0].items.some(item=>/cabeçalho|safe area/i.test(item)),'v66 notes must expose the affected mobile header/safe area');
assert.ok(releaseManifest.releases[0].items.some(item=>/Lista de compras|Mercado/i.test(item)),'v66 notes must mention the observed market-page seam');
assert.ok(releaseManifest.releases[0].items.some(item=>/não altera|apenas visual|apenas de apresentação|correção é apenas visual/i.test(item)),'v66 notes must state that data/financial state is untouched');
...
try {
  ...
  assert.match(index, /<meta name="app-build" content="v66"/);
  assert.match(index, /<meta name="theme-color" content="#f5f7fa"/);
  assert.match(index, /app-update\.css\?v=66/);
  assert.match(index, /app-update\.js\?v=66/);
  ...
  assert.match(index, /id="appBuildVersion">v66</);
  assert.match(events, /\.\/sw\.js\?v=66/);
  assert.equal(distManifest.latestVersion,'v66');
  assert.equal(distWebManifest.background_color,'#f5f7fa');
  assert.equal(distWebManifest.theme_color,'#f5f7fa');
  assert.ok(index.indexOf('sync.js?v=66') < index.indexOf('sync-conflict-policy.js?v=64-ui1'), 'sync conflict policy must load after the base sync engine');
  ...
  assert.ok(index.indexOf('v64-runtime.css?v=66-shell1') < index.indexOf('market-shopping-focus.css?v=65-shopping1'), 'v65 shopping focus remains the final market-specific layer');
...
console.log('Versioned v66 unified mobile shell, v65 shopping focus and controlled installation tests: OK');
