...
assert.equal(releaseManifest.latestVersion,'v71');
assert.ok(Array.isArray(releaseManifest.releases));
assert.equal(releaseManifest.releases[0].version,'v71');
...
assert.ok(releaseManifest.releases[0].items.some(item=>/off-canvas|deslizando|esquerda/i.test(item)),'v71 notes must expose the full drawer slide');
assert.ok(releaseManifest.releases[0].items.some(item=>/backdrop|fundo|blur/i.test(item)),'v71 notes must expose the progressive backdrop');
assert.ok(releaseManifest.releases[0].items.some(item=>/280|240|dura/i.test(item)),'v71 notes must document restrained timing');
assert.ok(releaseManifest.releases[0].items.some(item=>/reduced-motion|movimento reduzido/i.test(item)),'v71 notes must preserve reduced motion');
assert.ok(releaseManifest.releases[0].items.some(item=>/não modifica|exclusivamente|não altera/i.test(item)),'v71 notes must state that financial/private data is untouched');
...
assert.match(menuJs,/Conta de Casa v71/);
assert.match(menuCss,/Conta de Casa v71/);
...
try {
  ...
  assert.match(index, /<meta name="app-build" content="v71"/);
  assert.match(index, /<meta name="theme-color" content="#f5f7fa"/);
  assert.match(index, /app-update\.css\?v=71/);
  assert.match(index, /app-update\.js\?v=71/);
  ...
  assert.match(index, /id="appBuildVersion">v71</);
  assert.match(events, /\.\/sw\.js\?v=71/);
  assert.equal(distManifest.latestVersion,'v71');
  assert.equal(distWebManifest.background_color,'#f5f7fa');
  assert.equal(distWebManifest.theme_color,'#f5f7fa');
  assert.ok(index.indexOf('sync.js?v=71') < index.indexOf('sync-conflict-policy.js?v=64-ui1'), 'sync conflict policy must load after the base sync engine');
  ...
  assert.ok(index.indexOf('market-shopping-focus.css?v=65-shopping1') < index.indexOf('mobile-menu-toggle.css?v=71-menu5'), 'v71 global menu styling must load last');
  ...
  assert.ok(index.indexOf('market-shopping-focus.js?v=65-shopping1') < index.indexOf('mobile-menu-toggle.js?v=71-menu5'), 'v71 menu controller must run after the existing navigation/runtime listeners are defined');
...
console.log('Versioned v71 smooth drawer motion, v66 shell, v65 shopping focus and controlled installation tests: OK');
