...
assert.match(events,/__swAutoUpdateTimer/,'runtime must periodically check for newer builds while the app is open');
assert.match(events,/setInterval\([\s\S]{0,240}30000/,'automatic update polling must remain bounded to 30 seconds');
assert.match(events,/requestWaitingActivation/,'waiting workers must be activated without requiring the settings screen');
assert.match(events,/controllerchange[\s\S]{0,180}location\.reload\(\)/,'taking control with a new worker must reload the current page automatically');
...
try {
  ...
  assert.match(index, /invoice-capture\.js\?v=76-invoice-touch2/);
  assert.match(events, /\.\/sw\.js\?v=76-invoice-touch2/);
  ...
  assert.ok(index.indexOf('mobile-menu-toggle.js?v=73-menu8') < index.indexOf('v75-architecture.js?v=76-architecture-touch2'));
  assert.ok(index.indexOf('v75-architecture.js?v=76-architecture-touch2') < index.indexOf('v75-stability.js?v=75-stability1'));
