...
assert.equal(releaseManifest.latestVersion,'v68');
assert.ok(Array.isArray(releaseManifest.releases));
assert.equal(releaseManifest.releases[0].version,'v68');
...
assert.ok(releaseManifest.releases[0].items.some(item=>/painel|menu|drawer/i.test(item)),'v68 notes must expose the refined mobile drawer');
assert.ok(releaseManifest.releases[0].items.some(item=>/responsiv|largura|ecrã/i.test(item)),'v68 notes must expose responsive sizing');
assert.ok(releaseManifest.releases[0].items.some(item=>/48|táct|focus|hover/i.test(item)),'v68 notes must mention interaction/touch accessibility');
assert.ok(releaseManifest.releases[0].items.some(item=>/não modifica|exclusivamente|não altera/i.test(item)),'v68 notes must state that financial/private data is untouched');
...
assert.match(menuJs,/Conta de Casa v68/);
assert.match(menuCss,/Conta de Casa v68/);

assert.match(sw, /conta-de-casa-public-v64-runtime1-v65-shopping1-v66-shell1-v68-menu2/);
...
try {
  ...
  assert.match(index, /<meta name="app-build" content="v68"/);
  assert.match(index, /<meta name="theme-color" content="#f5f7fa"/);
  assert.match(index, /app-update\.css\?v=68/);
  assert.match(index, /app-update\.js\?v=68/);
  ...
  assert.match(index, /mobile-menu-toggle\.css\?v=68-menu2/);
  assert.match(index, /mobile-menu-toggle\.js\?v=68-menu2/);
  assert.doesNotMatch(index, /\?v=53/);
  assert.match(index, /id="appBuildVersion">v68</);
  assert.match(events, /\.\/sw\.js\?v=68/);
  assert.equal(distManifest.latestVersion,'v68');
  assert.equal(distWebManifest.background_color,'#f5f7fa');
  assert.equal(distWebManifest.theme_color,'#f5f7fa');
  assert.ok(index.indexOf('sync.js?v=68') < index.indexOf('sync-conflict-policy.js?v=64-ui1'), 'sync conflict policy must load after the base sync engine');
  ...
  assert.ok(index.indexOf('market-shopping-focus.css?v=65-shopping1') < index.indexOf('mobile-menu-toggle.css?v=68-menu2'), 'v68 global menu styling must load last');
  ...
  assert.ok(index.indexOf('market-shopping-focus.js?v=65-shopping1') < index.indexOf('mobile-menu-toggle.js?v=68-menu2'), 'v68 menu controller must run after the existing navigation/runtime listeners are defined');
...
console.log('Versioned v68 refined mobile menu, v66 shell, v65 shopping focus and controlled installation tests: OK');
