...
assert.ok(releaseManifest.releases[0].items.some(item=>/Lucide|ícone|hidrata/i.test(item)),'v72 notes must expose icon ownership stabilization');
assert.ok(releaseManifest.releases[0].items.some(item=>/hambúrguer|desenho próprio|custom/i.test(item)),'v72 notes must expose animated-menu protection');
...
assert.match(iconJs,/Conta de Casa v72/);

assert.match(sw, /conta-de-casa-public-v64-runtime1-v65-shopping1-v66-shell1-v71-menu5-v72-icons1/);
for(const asset of ['./app-update.css','./app-update.js','./ui-consistency.css','./v64-runtime.css','./v64-runtime.js','./market-shopping-focus.css','./market-shopping-focus.js','./mobile-menu-toggle.css','./mobile-menu-toggle.js','./ui-icons.css','./ui-icons.js','./release-manifest.json'])assert.ok(sw.includes(`'${asset}'`),`${asset} must be cached`);
...
for(const asset of ['app-update.css','app-update.js','ui-consistency.css','v64-runtime.css','v64-runtime.js','market-shopping-focus.css','market-shopping-focus.js','mobile-menu-toggle.css','mobile-menu-toggle.js','ui-icons.css','ui-icons.js','release-manifest.json'])assert.ok(prepare.includes(`'${asset}'`),`${asset} must be copied to dist`);
...
try {
  ...
  assert.ok(index.indexOf('market-shopping-focus.css?v=65-shopping1') < index.indexOf('mobile-menu-toggle.css?v=71-menu5'), 'menu layer must load after market-specific styling');
  ...
  assert.ok(index.indexOf('market-shopping-focus.js?v=65-shopping1') < index.indexOf('mobile-menu-toggle.js?v=71-menu5'), 'menu controller must run after the existing navigation/runtime listeners are defined');
  for(const asset of ['app-update.css','app-update.js','ui-consistency.css','v64-runtime.css','v64-runtime.js','market-shopping-focus.css','market-shopping-focus.js','mobile-menu-toggle.css','mobile-menu-toggle.js','ui-icons.css','ui-icons.js','release-manifest.json'])assert.ok(fs.existsSync(path.join(dist,asset)),`${asset} must exist in dist`);
} finally {
  fs.rmSync(dist, { recursive:true, force:true });
}

console.log('Versioned v72 icon ownership, mobile-menu continuity, shell, shopping focus and controlled installation tests: OK');
