...
assert.equal(releaseManifest.latestVersion,'v67');
assert.ok(Array.isArray(releaseManifest.releases));
assert.equal(releaseManifest.releases[0].version,'v67');
assert.ok(releaseManifest.releases[0].items.length>=6);
...
assert.ok(releaseManifest.releases[0].items.some(item=>/hambúrguer|menu/i.test(item)),'v67 notes must expose the animated mobile menu');
assert.ok(releaseManifest.releases[0].items.some(item=>/X|fechar/i.test(item)),'v67 notes must expose the close state');
assert.ok(releaseManifest.releases[0].items.some(item=>/44|tátil|aria/i.test(item)),'v67 notes must mention the accessible touch target/state');
assert.ok(releaseManifest.releases[0].items.some(item=>/não modifica|exclusivamente|não altera/i.test(item)),'v67 notes must state that financial/private data is untouched');
...
assert.match(menuJs,/Conta de Casa v67/);
...
try {
  ...
  assert.match(index, /<meta name="app-build" content="v67"/);
  assert.match(index, /<meta name="theme-color" content="#f5f7fa"/);
  assert.match(index, /app-update\.css\?v=67/);
  assert.match(index, /app-update\.js\?v=67/);
  ...
  assert.match(index, /id="appBuildVersion">v67</);
  assert.match(events, /\.\/sw\.js\?v=67/);
  assert.equal(distManifest.latestVersion,'v67');
  assert.equal(distWebManifest.background_color,'#f5f7fa');
  assert.equal(distWebManifest.theme_color,'#f5f7fa');
  assert.ok(index.indexOf('sync.js?v=67') < index.indexOf('sync-conflict-policy.js?v=64-ui1'), 'sync conflict policy must load after the base sync engine');
  ...
  assert.ok(index.indexOf('market-shopping-focus.css?v=65-shopping1') < index.indexOf('mobile-menu-toggle.css?v=67-menu1'), 'v67 global menu styling must load last');
  ...
  assert.ok(index.indexOf('market-shopping-focus.js?v=65-shopping1') < index.indexOf('mobile-menu-toggle.js?v=67-menu1'), 'v67 menu controller must run after the existing navigation/runtime listeners are defined');
...
console.log('Versioned v67 animated mobile menu, v66 shell, v65 shopping focus and controlled installation tests: OK');
