...
assert.equal(releaseManifest.latestVersion,'v72');
assert.ok(Array.isArray(releaseManifest.releases));
assert.equal(releaseManifest.releases[0].version,'v72');
...
assert.ok(releaseManifest.releases[0].items.some(item=>/página|backdrop|área visível/i.test(item)),'v72 notes must expose page/backdrop swipe close');
assert.ok(releaseManifest.releases[0].items.some(item=>/dedo|deslizar|swipe|gesto/i.test(item)),'v72 notes must expose direct gesture feedback');
assert.ok(releaseManifest.releases[0].items.some(item=>/vertical|intenção horizontal/i.test(item)),'v72 notes must preserve vertical movement safety');
assert.ok(releaseManifest.releases[0].items.some(item=>/não modifica|exclusivamente|não altera/i.test(item)),'v72 notes must state that financial/private data is untouched');
...
assert.match(menuJs,/Conta de Casa v72/);
assert.match(menuCss,/Conta de Casa v71/,'v72 reuses the validated v71 visual layer and only revises the controller');

assert.match(sw, /conta-de-casa-public-v64-runtime1-v65-shopping1-v66-shell1-v72-menu6/);
...
try {
  ...
  assert.match(index, /<meta name="app-build" content="v72"/);
  assert.match(index, /<meta name="theme-color" content="#f5f7fa"/);
  assert.match(index, /app-update\.css\?v=72/);
  assert.match(index, /app-update\.js\?v=72/);
  ...
  assert.match(index, /mobile-menu-toggle\.css\?v=72-menu6/);
  assert.match(index, /mobile-menu-toggle\.js\?v=72-menu6/);
  assert.doesNotMatch(index, /\?v=53/);
  assert.match(index, /id="appBuildVersion">v72</);
  assert.match(events, /\.\/sw\.js\?v=72/);
  assert.equal(distManifest.latestVersion,'v72');
  assert.equal(distWebManifest.background_color,'#f5f7fa');
  assert.equal(distWebManifest.theme_color,'#f5f7fa');
  assert.ok(index.indexOf('sync.js?v=72') < index.indexOf('sync-conflict-policy.js?v=64-ui1'), 'sync conflict policy must load after the base sync engine');
  ...
  assert.ok(index.indexOf('market-shopping-focus.css?v=65-shopping1') < index.indexOf('mobile-menu-toggle.css?v=72-menu6'), 'v72 menu layer must load last');
  ...
  assert.ok(index.indexOf('market-shopping-focus.js?v=65-shopping1') < index.indexOf('mobile-menu-toggle.js?v=72-menu6'), 'v72 menu controller must run after the existing navigation/runtime listeners are defined');
...
console.log('Versioned v72 page-swipe return, v66 shell, v65 shopping focus and controlled installation tests: OK');
