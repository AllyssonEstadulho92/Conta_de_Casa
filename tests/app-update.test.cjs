...
assert.equal(releaseManifest.latestVersion,'v64');
assert.ok(Array.isArray(releaseManifest.releases));
assert.equal(releaseManifest.releases[0].version,'v64');
...
try {
  ...
  assert.match(index, /<meta name="app-build" content="v64"/);
  assert.match(index, /app-update\.css\?v=64/);
  assert.match(index, /app-update\.js\?v=64/);
  ...
  assert.match(index, /id="appBuildVersion">v64</);
  assert.match(events, /\.\/sw\.js\?v=64/);
  assert.equal(distManifest.latestVersion,'v64');
  assert.ok(index.indexOf('sync.js?v=64') < index.indexOf('sync-conflict-policy.js?v=64-ui1'), 'sync conflict policy must load after the base sync engine');
  ...
  assert.ok(index.indexOf('ui-consistency.css?v=64-ui1') < index.indexOf('v64-runtime.css?v=64-runtime1'), 'v64 safe-area CSS must load last');
...
console.log('Versioned v64 software update center, release manifest and controlled installation tests: OK');
