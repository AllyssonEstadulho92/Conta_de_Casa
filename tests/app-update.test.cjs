...
assert.ok(releaseManifest.releases[0].items.some(item=>/desaparecia|salto visual|Safari/i.test(item)),'v72 notes must expose the menu-button handoff correction');
assert.ok(releaseManifest.releases[0].items.some(item=>/placeholder|44 por 44/i.test(item)),'v72 notes must expose stable topbar geometry');
assert.ok(releaseManifest.releases[0].items.some(item=>/showModal|frame seguinte|estado fechado/i.test(item)),'v72 notes must document staged modal opening');
...
try {
  ...
  assert.ok(index.indexOf('market-shopping-focus.css?v=65-shopping1') < index.indexOf('mobile-menu-toggle.css?v=72-menu7'), 'v72 menu layer must load last');
  ...
  assert.ok(index.indexOf('market-shopping-focus.js?v=65-shopping1') < index.indexOf('mobile-menu-toggle.js?v=72-menu7'), 'v72 menu controller must run after the existing navigation/runtime listeners are defined');
...
console.log('Versioned v72 stable menu handoff, v66 shell, v65 shopping focus and controlled installation expectations: OK');