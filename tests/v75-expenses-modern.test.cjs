...
assert.match(css,/76-canonical-expense-flow1/,'Despesas deve declarar a autoridade canónica da v76.');
...
assert.match(css,/@media\(max-width:820px\)[\s\S]*\.bill-filter-grid\{[\s\S]*display:flex!important[\s\S]*overflow-x:auto!important/,'No mobile os filtros devem usar progressive disclosure horizontal em vez de ocupar várias linhas.');
assert.match(css,/scroll-snap-type:x proximity/,'Filtros mobile devem manter navegação horizontal previsível.');
assert.match(css,/\.bill-filter-field\{[\s\S]*flex:0 0 min\(72vw,260px\)/,'Cada filtro mobile deve ter largura estável e legível.');
assert.match(css,/\.bill-filter-clear\{[\s\S]*min-height:44px!important/,'Limpar filtros mantém touch target acessível.');
