...
assert.match(js,/RETAILER_KEY='cdc_public_store_choice_v1'/,'retailer preference must use a storage key allowed by the plaintext guard');
assert.match(js,/LEGACY_RETAILER_KEY='cdc\.market\.scan\.retailer\.v1'/,'legacy retailer preference remains readable for migration');
assert.match(js,/localStorage\.getItem\(RETAILER_KEY\)\|\|localStorage\.getItem\(LEGACY_RETAILER_KEY\)/,'retailer preference must read the new key before the legacy fallback');
