...
assert.match(js,/76-expense-ios-tab-direct3/,'direct iOS tab binding must remain explicit');
assert.match(js,/function activateBillModeButton\(button,event\)/,'invoice modes must share one direct activation path');
assert.match(js,/function bindBillTabs\(form\)/,'invoice mode buttons must be bound directly when created');
assert.match(js,/button\.addEventListener\('touchend',activate,\{passive:true\}\)/,'iOS touchend must stay directly attached without cancelling user activation');
assert.match(js,/button\.addEventListener\('click',activate\)/,'desktop and synthetic clicks must stay on the same direct control');
assert.doesNotMatch(js,/document\.addEventListener\('touchend'/,'invoice tabs must no longer depend on document-level touch delegation');
