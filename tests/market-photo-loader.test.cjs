...
assert.match(source,/POLL_MS=1200/);
assert.match(source,/MAX_POLLS=10/);
assert.match(source,/MOBILE_PRIORITY_LIMIT=2/);
assert.match(source,/DESKTOP_PRIORITY_LIMIT=4/);
assert.match(source,/VIEWPORT_MARGIN_PX=160/);
assert.match(source,/RETRY_AFTER_MS=30000/);
assert.match(source,/LOADER_SETTLE_MS=12000/);
assert.match(source,/viewportCards/);
assert.match(source,/getBoundingClientRect/);
assert.match(source,/failedUntil=new Map/);
assert.match(source,/quarantineFailure/);
assert.match(source,/Fotografia temporariamente indisponível/);
...
assert.match(source,/observer\.observe\(marketPage/);
assert.match(source,/#page-market\.page\.active/);
assert.doesNotMatch(source,/CDCPingoDocePhotoLibrary\?\.warmPending/);
assert.doesNotMatch(source,/CDCPingoDocePhotoLibrary\?\.syncNow/);
assert.doesNotMatch(source,/cards\.slice\(0,18\)\.map\(hydrateCard\)/);
...
console.log('Market photo loader3 limits real viewport work, quarantines broken images and remains non-financial: OK');
